...
from . import db, enrich, hotspots, ingest, rare, seasonality, targets
...
def run(
    ...
    with db.session(db_path) as conn, EBirdClient() as client:
        typer.echo(f"[1/6] ingesting {csv}…")
        s1 = ingest.ingest_csv(conn, client, csv)
        typer.echo(f"      {s1}")

        typer.echo(f"[2/6] fetching hotspots within {radius} km of ({lat}, {lon})…")
        s2 = hotspots.fetch_nearby(conn, client, lat, lon, radius_km=radius, days_back=days_back, force=force_refresh)
        typer.echo(f"      {s2}")

        typer.echo("[3/6] sampling historical obs for seasonality…")
        s3 = seasonality.compute_seasonality(conn, client, force=force_refresh)
        typer.echo(f"      {s3}")

        typer.echo("[4/6] computing target birds (hotspot − life list)…")
        s4 = targets.compute_targets(conn)
        typer.echo(f"      {s4}")

        typer.echo(f"[5/6] flagging rare targets (last {rare_days} days within {rare_radius} km)…")
        s5 = rare.mark_rare(conn, client, lat, lon, radius_km=rare_radius, days_back=rare_days)
        typer.echo(f"      {s5}")

        typer.echo("[6/6] enriching with Wikipedia…")
        s6 = enrich.enrich_targets(conn, force=force_refresh)
        typer.echo(f"      {s6}")
...
def status(db_path: Path = typer.Option(db.DEFAULT_DB_PATH, "--db")) -> None:
    ...
    with db.session(db_path) as conn:
        for table in ("observations", "hotspots", "hotspot_obs", "targets", "species_info", "species_seasonality", "taxonomy"):
